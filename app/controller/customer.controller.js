...
customerController.verifyOtp = async (req, res) => {
  try {
    const { email, otp, password, phone, dob, name, type } = req.body;
     ...
     const validTypes = ["signin", "login"];

    if (!type || !validTypes.includes(type)) {
      return res.status(400).json({
        status: false,
        message: "Type must be either 'signin' or 'login'"
      });
    }
    ...
    //     return res.status(400).json({
    //       status: false,
    //       message: "Name, password, phone required"
    //     });
    //   }

    //   const customerCount = await Customer.countDocuments();
    //   const customerId = `FBCUS${String(customerCount + 1).padStart(4, "0")}`;
    ...
    //     name,
    //     dob,
    //     password: new Customer().generateHash(password),
    //   });
  ...
  return res.status(200).json({
    ...
    // 🔥 MISSING RESPONSE FIX
    return res.status(400).json({
      status: false,
      message: "Invalid type. Use 'signin' or 'login'"
    });
