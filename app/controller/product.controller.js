...
// /* =========================================================
//    🔴 STOCK STATUS HELPER (NEW ADDITION)
//    =========================================================
//    Ye function sirf DISPLAY ke liye hai
//    DB ya stock calculation ko affect nahi karta
// ========================================================= */
...
//             stockLabel: `Only ${stock} meter available`,
//             displayStock: stock
//         };
//     }
...
//         stockLabel: `${STOCK_THRESHOLD}+ meter available`,
//         displayStock: STOCK_THRESHOLD
//     };
// };
...
//             obj.productVarieties = obj.productVarieties.map(variant => ({
//                 ...variant,
//                 ...getStockStatus(variant.stock) // 👈 MAIN MODIFICATION
//             }));
...
/* =========================================================
   ✅ STOCK STATUS HELPER (DISPLAY ONLY)
========================================================= */
const getStockStatus = (stock) => {
  stock = Number(stock);
...
productController.addProduct = async (req, res) => {
  try {
    ...
    const categoryData = [];
    for (const cat of parsedCategory) {
      const categoryDoc = await Category.findOne({ name: cat.name });
      if (!categoryDoc) {
    ...
    const prdCount = await Product.countDocuments();
    const productId = `PRD${uuidv4().substring(0, 4)}${String(prdCount + 1).padStart(4, "0")}`;

    const imageUrls = [];
    const folderName = `Products/${name}(${productId})/${color}`;
    for (const file of req.files.productImages) {
      imageUrls.push(await upload_on_cloud(file, folderName));
    }
...
productController.editProductById = async (req, res) => {
  try {
    ...
    const index = product.productVarieties.findIndex(
      (v) => v.color === color
    );
    if (index === -1) {
    ...
    if (price !== undefined) {
      product.productVarieties[index].pricepermeter = Number(price);
    }

    if (stock !== undefined) {
      product.productVarieties[index].stock = Number(stock);
    }
...
productController.addNewVariety = async (req, res) => {
  try {
    ...
    if (stock === undefined || !price || !color) {
    ...
    res.status(200).json({ status: true, message: "New variety added" });
  } catch (err) {
    res.status(500).json({ status: false, message: err.message });
  }
};
...
// productController.bulkUploadProducts = async (req, res) => {
//   try {
...
//       if (!item.name || !item.pricepermeter) continue;
...
//             pricepermeter: Number(item.pricepermeter),
...
productController.bulkUploadProducts = async (req, res) => {
  try {
    ...
    for (const item of products) {
      ...
      const productId = `PRD${uuidv4().substring(0, 8)}`;

      await Product.create({
        productId, // ✅ REQUIRED FIELD FIXED
        name: item.name,
        description: item.description || "",
        productVarieties: [
          {
    ...
    return res.status(200).json({
      message: `${inserted} products uploaded successfully`,
    });
